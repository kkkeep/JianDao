 ...
 * <p>
 * 根据 backgroundview 创建 backgroundview,再根据backgroundview 创建 backgroundview，最后 windowmanager add  decorview
 * <p>
 * <p>
 ...
 * 该pop 可以显示 一下几个功能：<p/>
 *
 * 1. 可以设置pop 外部区域点击时是否关闭pop ：{@link #setTouchOutsideDismiss(boolean)}<p/>
 * 2. 可以设置按返回键是否可以关闭pop : {@link #setOnBackKeyDismiss(boolean)}<p/>
 * 3. 可是设置点击pop 外部时事件是否传递到 pop 的后面 : {@link #setOutsideCanTouch(boolean)}<p/>
 *
 *
 *
 */
...
public class CommonPopView extends PopupWindow {
    ...
    public CommonPopView(Context context, int width, int height) {
        super(context);
    ...
    protected void init(Context context) {
        ...
        /**
         * 设置widow 是否获取焦点，默认false,如果为false,那么点击pop以外的区域，事件会传递到popu后面的（behind）界面上.点击pop 以外的区域pop 不会消失，注意如果pop 里面有输入框，那么输入框不能弹出键盘
     ...
     * <p>
     * 注意：如果 设置为true,那么设置点击外部关闭 pop 将会无效，因此在为true 相关关闭 pop ,必须手动关闭,调用close
     *
     * @param touch
     */
